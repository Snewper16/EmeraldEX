 ...
 * Task_NewGameBirchSpeech_StartBirchOshawottPlatformFade
 * Task_NewGameBirchSpeech_StartBirchOshawottPlatformFade
 ...
 * Task_NewGameBirchSpeech_ReshowBirchOshawott
 ...
 *  - Advances to Task_NewGameBirchSpeechSub_WaitForOshawott
 * Task_NewGameBirchSpeechSub_WaitForOshawott
...
static void Task_NewGameBirchSpeechSub_WaitForOshawott(u8);
static void Task_NewGameBirchSpeech_StartBirchOshawottPlatformFade(u8);
...
static void Task_NewGameBirchSpeech_ReshowBirchOshawott(u8);
...
static const u8 *const sMalePresetNames[] = {
    COMPOUND_STRING("CONNOR"),
    COMPOUND_STRING("LOGAN"),
...
static const u8 *const sFemalePresetNames[] = {
    COMPOUND_STRING("CONNIE"),
    COMPOUND_STRING("MILLE"),
    COMPOUND_STRING("ALPHA"),
    COMPOUND_STRING("SAPPHIRE"),
    COMPOUND_STRING("EMERALD"),
    COMPOUND_STRING("MAY"),
    COMPOUND_STRING("HELEN"),
    COMPOUND_STRING("JILL"),
    COMPOUND_STRING("ASUKA"),
    COMPOUND_STRING("VALERIE"),
    COMPOUND_STRING("WEISS"),
    COMPOUND_STRING("MEI"),
    COMPOUND_STRING("NADINE"),
    COMPOUND_STRING("DAWN"),
    COMPOUND_STRING("MIZUKI"),
    COMPOUND_STRING("NICOLE"),
    COMPOUND_STRING("LILLIE"),
    COMPOUND_STRING("LIKO"),
    COMPOUND_STRING("LUCY"),
    COMPOUND_STRING("IRIS"),
    COMPOUND_STRING("ADORA")
};
...
#define tOshawottSpriteId data[9]
...
{
    u8 spriteId = gTasks[sBirchSpeechMainTaskId].tOshawottSpriteId;
    ...
    CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 112, 58, 0, 0, 32, PALETTES_BG, SPECIES_OSHAWOTT);
    gTasks[taskId].func = Task_NewGameBirchSpeechSub_WaitForOshawott;
    gTasks[sBirchSpeechMainTaskId].tTimer = 0;
}

static void Task_NewGameBirchSpeechSub_WaitForOshawott(u8 taskId)
{
    s16 *data = gTasks[taskId].data;
    struct Sprite *sprite = &gSprites[gTasks[sBirchSpeechMainTaskId].tOshawottSpriteId];
...
static void Task_NewGameBirchSpeech_AndYouAre(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        ...
        AddTextPrinterForMessage(TRUE);
        gTasks[taskId].func = Task_NewGameBirchSpeech_StartBirchOshawottPlatformFade;
    }
}

static void Task_NewGameBirchSpeech_StartBirchOshawottPlatformFade(u8 taskId)
{
    if (!RunTextPrintersAndIsPrinter0Active())
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        gSprites[gTasks[taskId].tOshawottSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static void Task_NewGameBirchSpeech_StartPlayerFadeIn(u8 taskId)
{
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[gTasks[taskId].tOshawottSpriteId].invisible = TRUE;
        if (gTasks[taskId].tTimer)
        {
            gTasks[taskId].tTimer--;
        }
...
static void Task_NewGameBirchSpeech_StartNamingScreen(u8 taskId)
{
    ...
    {
        FreeAllWindowBuffers();
        FreeAndDestroyMonPicSprite(gTasks[taskId].tOshawottSpriteId);
...
static void Task_NewGameBirchSpeech_SlidePlatformAway2(u8 taskId)
{
    ...
    {
        gTasks[taskId].func = Task_NewGameBirchSpeech_ReshowBirchOshawott;
    }
}

static void Task_NewGameBirchSpeech_ReshowBirchOshawott(u8 taskId)
{
    u8 spriteId;
    ...
    if (gTasks[taskId].tIsDoneFadingSprites)
    {
        ...
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
        spriteId = gTasks[taskId].tOshawottSpriteId;
        gSprites[spriteId].x = 100;
        gSprites[spriteId].y = 75;
        gSprites[spriteId].invisible = FALSE;
        gSprites[spriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static void Task_NewGameBirchSpeech_WaitForSpriteFadeInAndTextPrinter(u8 taskId)
{
    ...
    {
        gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        gSprites[gTasks[taskId].tOshawottSpriteId].oam.objMode = ST_OAM_OBJ_NORMAL;
        if (!RunTextPrintersAndIsPrinter0Active())
        {
            gSprites[gTasks[taskId].tBirchSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
            gSprites[gTasks[taskId].tOshawottSpriteId].oam.objMode = ST_OAM_OBJ_BLEND;
...
static u8 NewGameBirchSpeech_CreateOshawottSprite(u8 x, u8 y)
{
    return CreateMonPicSprite_Affine(SPECIES_OSHAWOTT, FALSE, 0, MON_PIC_AFFINE_FRONT, x, y, 14, TAG_NONE);
}
